...
def random_flip(image, gt_pts, gt_corners, crop_center=None):
    # image: h x w x c
    # gt_pts: num_obj x 4 x 2
    # gt_corners: num_pts x 2
    h, w, c = image.shape
    ...
    if np.random.random() < 0.5:
        ...
        if gt_corners.shape[0]:
            gt_corners[:,1] = h-1 - gt_corners[:,1]
        if crop_center is not None:
            crop_center[1] = h-1 - crop_center[1]
    return image, gt_pts, gt_corners, crop_center
...
def load_affine_matrix(crop_center, crop_size, dst_size, inverse=False, rotation=False):
    dst_center = np.array([np.round(dst_size[0]//2), np.round(dst_size[1]//2)], dtype=np.float32)

    if rotation and np.random.rand(1) > 0.5:
        random_degree = np.random.rand(1)[0] * 360
    else:
        random_degree = 0.
