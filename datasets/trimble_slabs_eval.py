...
def voc_eval(results,
    ...
    for i, res in enumerate(results):
        objects = []
        for pts in res['gts']['pts']:
            object_struct = {}
            object_struct['bbox'] = np.array(pts, dtype=np.float32).reshape(1, 8)
            object_struct['difficult'] = 0
            objects.append(object_struct)

        bbox = np.array([x['bbox'] for x in objects], dtype=np.float32).reshape(-1, 8)
        difficult = np.array([x['difficult'] for x in objects]).astype(bool)
    ...
    for i, res in enumerate(results):
        confs = res['predictions']['dif']
        bboxes = res['predictions']['pts']

        image_ids.extend([i] * len(confs))
        confidence.extend(confs)
        BB.extend(bboxes)
    ...
    return np.mean(rec), np.mean(prec), np.mean(ap)
